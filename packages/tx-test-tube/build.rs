...
fn main() {
    ...
    let prebuilt_lib_dir = manifest_dir.join("libtxtesttube").join("artifacts");

    let lib_name = "tx-test-tube";
    ...
    let header = if std::env::var("DOCS_RS").is_ok() {
        manifest_dir
            .join("libtxtesttube")
            .join("artifacts")
            .join("libtxtesttube.docrs.h")
    } else {
        out_dir.join(format!("lib{}.h", lib_name))
    };
    // rerun when go code is updated
    println!("cargo:rerun-if-changed=./libtxtesttube");
...
fn build_libtxtesttube(out: PathBuf) {
    ...
    let tidy_status = Command::new("go")
        .current_dir(manifest_dir.join("libtxtesttube"))
        .arg("mod")
        .arg("tidy")
        .spawn()
        .unwrap()
